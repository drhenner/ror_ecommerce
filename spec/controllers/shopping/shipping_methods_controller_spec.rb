...
describe Shopping::ShippingMethodsController do
  ...
  it "update action should render edit template when model is invalid" do
    @variant2     = FactoryBot.create(:variant)
    @address      = FactoryBot.create(:address)
    @order        = FactoryBot.create(:order, :ship_address => @address)
    @order_item   = FactoryBot.create(:order_item, :order => @order, :variant => @variant)
    @order_item2   = FactoryBot.create(:order_item, :order => @order, :variant => @variant2)
