...
describe Variant, " instance methods" do
  ...
  context ".sold_out?" do
    it 'should be sold out' do
      inventory   = FactoryBot.create(:inventory, count_on_hand: 100, count_pending_to_customer: (100 - Variant::OUT_OF_STOCK_QTY))
      @variant    = FactoryBot.create(:variant,   inventory: inventory)
      expect(@variant.sold_out?).to be true
    end

    it 'should not be sold out' do
      inventory   = FactoryBot.create(:inventory, count_on_hand: 100, count_pending_to_customer: (99 - Variant::OUT_OF_STOCK_QTY))
      @variant    = FactoryBot.create(:variant,   inventory: inventory)
  ...
  context ".low_stock?" do
      it 'should be low stock' do
        inventory   = FactoryBot.create(:inventory, count_on_hand: 100, count_pending_to_customer: (101 - Variant::OUT_OF_STOCK_QTY))
        @variant    = FactoryBot.create(:variant,   inventory: inventory)
        expect(@variant.low_stock?).to be true
      end

      it 'should be low stock' do
        inventory   = FactoryBot.create(:inventory, count_on_hand: 100, count_pending_to_customer: (100 - Variant::LOW_STOCK_QTY))
        @variant    = FactoryBot.create(:variant,   inventory: inventory)
        expect(@variant.low_stock?).to be true
      end

      it 'should not be low stock' do
        inventory   = FactoryBot.create(:inventory, count_on_hand: 100, count_pending_to_customer: (99 - Variant::LOW_STOCK_QTY))
        @variant    = FactoryBot.create(:variant,   inventory: inventory)
  ...
  context ".product_name" do
    ...
    it 'should return the products name and subname' do
        @variant.name = nil
        @variant.product.name = 'product says hello'
        @variant.stubs(:primary_property).returns  FactoryBot.create(:variant_property, :description => 'pp_name')
  ...
  context ".brand_name" do
    it 'should return the variants subname' do
      brand     = FactoryBot.create(:brand, name: 'Reabok')
  ...
  context ".primary_property" do
    it 'should return the primary property' do
      ...
      property.stubs(:display_name).returns('Color')
      variant_prop1 = FactoryBot.create(:variant_property, variant: @variant, property: property, primary: true)
      variant_prop2 = FactoryBot.create(:variant_property, variant: @variant, property: property2, primary: false)
      @variant.variant_properties.push(variant_prop2)
      @variant.variant_properties.push(variant_prop1)
  ...
  context ".is_available?" do
    it "should be available" do
      ...
      inventory   = FactoryBot.create(:inventory, count_on_hand: 100, count_pending_to_customer: 99)
      @variant    = FactoryBot.create(:variant,   inventory: inventory)
      @variant.save
  ...
  context ".qty_to_add=(num)" do
    it "should update count_on_hand with qty_to_add" do
      ...
      inventory   = FactoryBot.create(:inventory, count_on_hand: 100, count_pending_to_customer: 50)
      @variant    = FactoryBot.create(:variant,   inventory: inventory)
...
describe Variant, "instance method" do
  ...
  context ".quantity_purchaseable" do
    it 'should be quantity_purchaseable' do
      inventory   = FactoryBot.create(:inventory, count_on_hand: 100, count_pending_to_customer: (98))
      @variant    = FactoryBot.create(:variant,   inventory: inventory)
...
describe Variant, "#admin_grid(product, params = {})" do
  it "should return variants for a specific product" do
    ...
    variant1 = FactoryBot.create(:variant, product: product)
    variant2 = FactoryBot.create(:variant, product: product)
