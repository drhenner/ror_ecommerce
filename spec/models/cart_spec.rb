...
describe Cart, ".sub_total" do
  ...
  before(:each) do
    @cart = FactoryBot.create(:cart_with_two_5_dollar_items)
  end
...
describe Cart, " instance methods" do
  ...
  context " add_items_to_checkout" do
    ...
    before(:each) do
      @order = FactoryBot.create(:in_progress_order)
    end
...
describe Cart, '' do
  ...
  before(:each) do
    @cart = FactoryBot.create(:cart_with_two_items)
  end
...
describe  ".merge_with_previous_cart! " do
  before(:each) do
    @user     = FactoryBot.create(:user)
    @variant1 = FactoryBot.create(:variant, price: 1.00)
    @variant2 = FactoryBot.create(:variant, price: 5.00)
    @variant3 = FactoryBot.create(:variant, price: 30.00)
    @cart       = FactoryBot.create(:cart, user: @user)
    @cart_item  = FactoryBot.create(:cart_item, cart: @cart, user: @user, variant: @variant1, quantity: 2)
  end

  context 'with each cart having one item' do
    it 'should add items from previous cart' do
      previous_cart = FactoryBot.create(:cart, user: @user)
      cart_item2    = FactoryBot.create(:cart_item, cart: previous_cart, user: @user, variant: @variant2)
  ...
  context 'with each cart having the same' do
    it 'should add items from previous cart' do
      previous_cart = FactoryBot.create(:cart, user: @user)
      cart_item2    = FactoryBot.create(:cart_item, cart: previous_cart, user: @user, variant: @variant1, quantity: 1)
      @cart.merge_with_previous_cart!
      @cart.reload
      expect(@cart.cart_items.map(&:variant_id).include?(@variant1.id)).to be true
...
describe Cart, ".shopping_cart_items_equal_order_items?" do
  ...
  context 'when variants are not equal' do
    before(:each) do
      variant_1 = FactoryBot.create(:variant)
      variant_2 = FactoryBot.create(:variant)
